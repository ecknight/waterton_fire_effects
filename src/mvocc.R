...
## p: availability from survival model
## lambda: Poisson mean from conditional likelihood step
## Y: M x J matrix; occupancy matrix
## X: M x nOP matrix; ???
## Z: MxJ x nDP matrix (visits stacked); covariates
...
method="Nelder-Mead", init, control=list(), hessian=TRUE, DElimit=10, ...) {
    ...
    ## detection part: p * q
    ## - p is already provided from surv model --> availability
    ## - q is whaterver else is there to make detectability less than (1-p)
    ## occupancy part: delta * G
    ## - G is estimated from the individual counts, and that is P(Y>0)=1-exp(-lambda)
    ## - delta is a 'zero inflation' term that related to suitability???
    nll <- function(params) {
        delta <- plogis(X %*% params[1:nOP]) #this is psi in regular model
        psi <- delta * (1-exp(-lambda)) #this is new
        pvec <- plogis(Z %*% params[(nOP + 1):nP]) * p #*p is added
        cp <- (pvec^yvec) * ((1 - pvec)^(1 - yvec))
        cp[navec] <- 1
        cpmat <- matrix(cp, M, J)
        loglik <- log(rowProds(cpmat) * psi + nd * (1 - psi))
    ...
    out <- list(coef=cf, loglik=ll, vcov=S, nobs=nrow(Y), df=length(cf),
        summary=coefs, init=init, method=method, lambda=lambda,
        results=opt,
        nobs=M,
        nvisits=J)
    class(out) <- "mvocc"
